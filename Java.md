...
4. default（同一包内可见）
###非访问修饰符
1. static 修饰符，静态变量只有一份拷贝,局部变量不能被声明为 static 变量。被static修饰的内部类可以直接作为一个普通类来使用，而不需先实例一个外部类。静态内部类只能访问外部类的静态成员。

2. final 修饰符，**final 修饰的类不能够被继承**，修饰的方法不能被继承类重新定义（覆写），修饰的变量为常量，是不可修改的，且必须要显示初始化。有两种初始化方式，一种是在变量声明的时候初始化；第二种是在在这个变量所在的类的所有的构造函数中对这个变量赋初值。
